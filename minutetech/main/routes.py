...
from ..forms import ContactForm, RegistrationForm, AskForm, EditAccountForm, PasswordResetForm, EmailResetForm, PhoneResetForm, ProfilePictureForm, photos
...
def login():
	...
	try:
		...
		if request.method == "POST":
			...
			x = c.execute("SELECT * FROM clients WHERE email = (%s)",(thwart(request.form['email']),))
			...
			if int(x) > 0:
				...
				if sha256_crypt.verify(request.form['password'], pdata):
					...
					error = "Invalid credentials, try again. Tries: {}".format(count)
			else:
