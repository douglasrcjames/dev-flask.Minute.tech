...
#############  TESTING B4 POST (all normal testing should be in the dev.minute.tech)  ##################
# TESTING route
	...
	try:
		...
		if request.method == "POST" and form.validate():
			...
			if int(x) > 0:
				...
				msg = Message("Minute.tech - Email Verification", sender = "test@minute.tech", recipients=[email])
